...
2. **捕捉資本結構動態。** 不同於簡單的價格比率，mNAV 納入了可轉債、優先擔保債券和優先股——這些都是 Strategy 大量發行以融資購買 BTC 的工具。這使 mNAV 同時對 BTC 價格變動和公司融資決策敏感，訊號比單純的 NAV Premium 更豐富。

3. **歷史極值提供可驗證的信號。** mNAV 歷史範圍從低於 1.0×（2022 年熊市）到超過 3.5×（2021 年牛市高峰）。這些極端值歷史上都先於均值回歸，使 mNAV 成為潛在的反向操作信號。

4. **機構層面的實務意義。** mNAV 是機構級比特幣儲備分析師在研究 Strategy 時採用的主要估值指標，具備實務重要性，而非純學術概念。
